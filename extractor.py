...
def folder_for_ext(ext: str, input_dir: Path) -> Optional[Path]:
    ...
    if ext in XLSX_EXT:  return input_dir / "docs"
    return None
...
def copy_and_sort(src: Path, input_dir: Path) -> Optional[Path]:
    ...
    Returns the new path, or None if unsupported.
    """
...
def unpack_archives_keep(input_dir: Path, depth: int = 0) -> List[Path]:
    ...
    Files are COPIED to the correct subfolders; archives are NEVER deleted.
...
class PDFPlumberTableExtractor:
    ...
    def _clean_headers(h):
        return [
            str(v).strip() if v and str(v).strip() else f"column_{i}"
            ...
            with pdfplumber.open(pdf_path) as pdf:
                for p_idx, page in enumerate(pdf.pages):
                    for t_idx, table in enumerate(page.extract_tables() or []):
                        if not self._is_valid(table):
                            continue
                        headers = self._clean_headers(table[0])
                        n = len(headers)
                        rows = [
                            self._fix_colspan(self._normalize_row(r, n))
                            for r in table[1:]
                        ]
